...
- Evidence assembly reads: define/spec (XML/HTML/XLSX), protocol text, aCRF index CSV,
  TLF titles, USDM design, and ARS/ARD JSONs.
...
- Table-only tweak (requested): Enforce edge direction upstream→downstream:
  Protocol/USDM → CRF → SDTM → ADaM → TLF.
- FINAL PASS (requested): prune ALL orphan nodes (any type) before returning.
"""
...
def _build_messages_for_ars_backtrace(tlf_cell_id: str, adam_vars: List[str], retrieved: List[Dict[str, str]]) -> List[Dict[str, str]]:
    ...
    SYSTEM = (
        ...
        "Output STRICT JSON ONLY in this schema:\n"
        "{ 'variable': '<display id or cell id>', 'dataset': 'table', 'summary': '<brief>', "
...
def _build_messages_for_ars_cell(cell_spec: str, retrieved: List[Dict[str, str]]) -> List[Dict[str, str]]:
    SYSTEM = (
        ...
        "Every 'explanation' MUST begin with [direct]/[reasoned]/[general].\n"
...
def _build_messages_for_ars_connectivity_repair(tlf_cell_id: str,
                                                adam_vars: List[str],
                                                orphan_sdtm_vars: List[str],
                                                retrieved: List[Dict[str, str]]) -> List[Dict[str, str]]:
    SYSTEM = (
        "You are repairing an ARS-derived backtrace graph. Some SDTM variables appear but are not linked "
        "to any ADaM parents. Your job: for EACH listed SDTM variable, either (1) add the missing, "
        "evidence-based connection(s) to one or more of the already-linked ADaM variables (adding necessary ADaM helper "
        "variables like BASE/CHG/AVISIT/ANLxxFL/PARAMCD as needed), or (2) if no evidence supports the link, "
        "do not connect it and instead return a 'gaps' note explaining exactly why. "
        "Do NOT create unrelated nodes. Keep everything CONNECTED up to CRF and Protocol anchors if available. "
        "Every explanation must start with [direct]/[reasoned]/[general] and include a short anchor/citation, as a full sentence.\n"
...
def _auto_prefix_adam_vars(graph: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for n in lin:
        ...
        if t in ("adam dataset", "adam data set", "adaml dataset", "adml dataset"):
            ds_ids.add(n.get("id"))
        nid = (n.get("id") or "")
...
# --- Final pass: remove ALL orphan nodes (any type) ---------------------------
def _prune_all_orphans(graph: Dict[str, Any]) -> Dict[str, Any]:
    lin = graph.setdefault("lineage", {}).setdefault("nodes", [])
    edges = graph.setdefault("lineage", {}).setdefault("edges", [])
    gaps = graph.setdefault("lineage", {}).setdefault("gaps", [])

    if not lin:
        return graph

    # Keep single-node/no-edge fallbacks intact.
    if not edges:
        return graph
    ...
    for n in lin:
        nid = str(n.get("id") or "")
        if nid and nid in connected:
            kept.append(n)
        else:
            removed.append(nid or "<empty-id>")

    if removed:
        graph["lineage"]["nodes"] = kept
        graph["lineage"]["edges"] = [
            e for e in edges
            if str(e.get("from") or "") in connected and str(e.get("to") or "") in connected
        ]
        gaps.append({"explanation": f"Removed {len(removed)} orphan node(s): {', '.join(removed)}."})
...
def _augment_backtrace_if_missing_sdtm(base_graph: Dict[str, Any], *, model: str, embed_model: str) -> Dict[str, Any]:
    ...
    query = f"Backtrace ADaM vars {', '.join(adam_vars)} to SDTM→CRF→Protocol for output '{base_graph.get('variable')}'."
...
def _ensure_ars_connectivity(graph: Dict[str, Any], *, model: str, embed_model: str) -> Dict[str, Any]:
    try:
        ...
        touch = set()
        for e in edges:
            if e.get("from") in sdtm_vars and e.get("to") in adam_vars:
                touch.add(e.get("from"))
            if e.get("to") in sdtm_vars and e.get("from") in adam_vars:
                touch.add(e.get("to"))
...
# ---------------- main builders ----------------
def build_lineage_with_llm_from_session(dataset: str,
                                        variable: str,
                                        files_ctx: Optional[List[Dict[str, Any]]] = None,
                                        *,
                                        model: str = DEFAULT_MODEL,
                                        embed_model: str = EMBED_MODEL) -> Dict[str, Any]:
    ...
    if not chunks:
        out = {
            ...
            "lineage": {
                "nodes": [{"id": f"{dataset}.{variable}".upper(),
                           "type": _suggest_type_for_id(f"{dataset}.{variable}", dataset),
                           "explanation": "[general] Target only; no artifacts to trace against."}],
                "edges": [],
                "gaps": ["No evidence found."]
            }
        }
        return _prune_all_orphans(out)  # final prune (no-op here)
    ...
    try:
        ...
        # FINAL: remove all orphan nodes (e.g., duplicate SDTM/CRF/protocol/tlf nodes not connected)
        return _prune_all_orphans(out)
...
def build_endpoint_lineage_with_llm_from_session(endpoint_term: str,
                                                 files_ctx: Optional[List[Dict[str, Any]]] = None,
                                                 *,
                                                 model: str = DEFAULT_MODEL,
                                                 embed_model: str = EMBED_MODEL) -> Dict[str, Any]:
...
def build_table_lineage_from_session(display_spec: str,
                                     files_ctx: Optional[List[Dict[str, Any]]] = None,
                                     *,
                                     model: str = DEFAULT_MODEL,
                                     embed_model: str = EMBED_MODEL) -> Dict[str, Any]:
    # ARS-only LLM cell-matcher path
    if _is_cell_spec(display_spec):
        base_graph = _build_ars_cell_base_graph_llm(display_spec, model=model, embed_model=embed_model)
        ...
        return _prune_all_orphans(base_graph)

    # Display-level path (titles/define/ARS summary)
    sess = _latest_session()
    summary = _load_session_summary(sess)
